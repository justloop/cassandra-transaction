...
CREATE TABLE d8.order2 (o_w_id int, o_d_id int, o_id int, o_c_id int, c_first text, c_middle text, c_last text, o_carrier_id int, o_ol_cnt int, o_all_local int, o_entry_d timestamp,ol_delivery_d timestamp, ols set<frozen <orderline>>, PRIMARY KEY((o_w_id, o_d_id), o_id, o_c_id)) WITH CLUSTERING ORDER BY (o_id ASC, o_c_id DESC);
